...
| E1.4 | `service_edges` hourly derivation job | M | W2 | done (PR #14) |
| E1.5 | Retention job (24 h for non-scenario rows) | M | W2 | done (PR #14) |
| E1.6 | Scenario capture: tag rows in [start, end] with scenario_id; export fixture (`.sql.gz`) | M/S | W7 | done early (PR #26; fixture is `.jsonl.gz`, imported by our own code) |

### E2 — Change events and alerting
| ID | Feature | Pri | Week | Status |
|---|---|---|---|---|
| E2.1 | flagd config watcher → `change_events(type=flag)` | M | W2 | done (PR #15) |
| E2.2 | Deploy/restart/scale events from compose labels + action executor | M | W2/W6 | observation half done (PR #17); executor W6 |
| E2.3 | Alert rules table + evaluator (5xx rate, p95 vs baseline, memory, Kafka lag) | M | W2 | done (PR #16) |
| E2.4 | Incident lifecycle: open → investigating → awaiting_approval → remediating → resolved/failed | M | W2 | done (PR #15: tables, state machine, read API; evaluator opens incidents in E2.3) |
...
| E3.4 | `correlate_changes` node | M | W4 | done (PR #23, deterministic; no summarising call) |
...
| ID | Feature | Pri | Week | Status |
|---|---|---|---|---|
| E4.1 | Verifier: every evidence ref exists; numeric claims within ±20%; drops failures, recomputes confidence | M | W4 | done (PR #23) |
| E4.2 | Verifier unit tests incl. adversarial fabricated refs | M | W4 | done (PR #23, 100 % line coverage) |
...
| ID | Feature | Pri | Week | Status |
|---|---|---|---|---|
| E7.1 | `bench/scenarios.yaml`: 12 faults + 3 noise + S13 injection, expected category/service/action | M | W4/W8 | done (PR #26; S9–S12 overlays are placeholders until E7.3) |
| E7.2 | Scenario runner (toggle flags / apply overlays / wait / capture) | M | W4 | done for flag faults + S13 injection (PR #26); overlays with E7.3 |
...
| E11.5 | Supabase keep-alive cron (free tier pauses after 7 days idle) | M | W1 | done (PR #29: uptime cron requires /readyz) |
...
| ID | Feature | Pri | Week | Status |
|---|---|---|---|---|
| E12.1 | README (§18.1 structure) | M | W13 | living since PR #24 (results/GIF/video slots marked "not yet"); polish W13 |
...
| 1.0.9 | 19 Sep 2026 | W2 D1: E1.5 retention (`run_retention`, untagged rows older than `AEGIS_RETENTION_HOURS`=24, hourly) and E1.4 `service_edges` (migration `0002`; parent→child span pairs across services, per hour: call_count, err_count, p95 of the caller-side duration; delete-then-insert per window; refreshed every 15 min for current+previous hour). Periodic jobs run in a ~40-line asyncio `JobRunner` started in the lifespan (own code, not APScheduler yet; revisit at E2.3). Admin routes `POST /api/v1/admin/{retention,service-edges}/run` behind `X-Admin-Token` (`AEGIS_ADMIN_TOKEN`; unset → 503 fail-closed). 36 tests, 98 %. Live check against the demo: 16 edges in the first 3 min (e.g. frontend-proxy→frontend 137 calls p95 308 ms, frontend→product-catalog 67 calls p95 608 ms, load-generator→flagd p95 1.1 ms); runner ticks every 60 s in the test run, 15 min by default. Note: the admin retention test runs for real against the shared local DB, so `make check` on the laptop deletes untagged rows older than 24 h (CI uses a fresh DB). |
| 1.0.10 | 19 Sep 2026 | W2 D1 (2nd PR): E2.4 migration `0003` (`change_events`, `alert_rules`, `incidents`; enums as VARCHAR+CHECK), incident state machine (`incidents/lifecycle.py`, `active_incident` dedupe), `GET /api/v1/incidents[/{id}]` with keyset pagination. E2.1 `FlagWatcher`: mtime poll of `demo.flagd.json` (`AEGIS_FLAGD_CONFIG_PATH`, 2 s), diff of `defaultVariant`, `change_events(type=flag, actor=flagd-file)` with service from `config/targets/otel-demo.yaml` (new: target-specific names live there, per 1.0.3). Live: toggle → row in 1.7 s. **Bug found live and fixed:** `JobRunner.run_once` returned from inside `async for` over the `session_scope` generator, closing it before its commit — every runner tick since PR #14 logged success and persisted nothing (admin endpoints were fine: FastAPI drives generators to completion). `session_scope` is now an `@asynccontextmanager`; `job.ok` logs after commit; regression test reads back from a fresh session. 53 tests, 97 %. `pyyaml` added to the API. |
...
| 1.0.19 | 26 Sep 2026 | W3 D5: Supabase (Postgres 17.6, ap-southeast-1) migrated to `0006` via the **Session pooler** (direct host is IPv6-only; Cloud Run egress is IPv4). Secret Manager: `aegis-database-url`, `aegis-admin-token` (new, prod-only), `aegis-gemini-api-key`, `aegis-groq-api-key`. `deploy-api.yml`: `--set-secrets`, `AEGIS_PUBLIC_MODE=true`, `/readyz` now a hard traffic gate. E11.5: uptime cron requires `/readyz` (keep-alive). Langfuse keys verified locally (E8.1 next). **First deploy against Supabase failed at startup** (`config/` missing from the image; only the with-DB path reads it); the startup probe kept traffic on the old revision. PR #30 ships `config/` and makes CI boot the image against Postgres and require readyz 200 + agent.ready + rules seeded. **Live (rev `00032`):** livez/readyz 200 (database true), `agent.ready` + `alerts.rules_seeded` logged, incidents/scenarios 200 empty, admin 401 without token, uptime run green. Gap noted: public run start not yet rate-limited (FR-16); must land before importing a scenario. |
